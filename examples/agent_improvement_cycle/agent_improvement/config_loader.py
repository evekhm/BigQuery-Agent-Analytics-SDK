...
The config file lives at ``<agent_root>/improve/config.json``.
All relative paths inside it are resolved against ``<agent_root>``
(the config file's grandparent directory).
...
def load_agent_module(config_path: str) -> tuple:
  ...
  agent_root = os.path.dirname(os.path.dirname(config_path))
...
def load_config(config_path: str) -> ImprovementConfig:
  ...
  return ImprovementConfig(
      ...
      prompt_adapter=PythonFilePromptAdapter(
          cfg["prompts_path"],
